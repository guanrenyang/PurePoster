...
PurePoster is a fancy tool for making **arbitrary-resolutional, simple-style posters** or wallpapers with ***text in center***.

## Functionality

PurePoster could generate simple-stype posters or wallpapers with text in the centered. You can generate high-resolution
wallpapers through ***specifying the resolution according to the that of your display device***. 
...
For online display, the example is in `jpeg` format, so it is rather vague. The lossless compressed `bmp` format can be seen by clicking the link below.
